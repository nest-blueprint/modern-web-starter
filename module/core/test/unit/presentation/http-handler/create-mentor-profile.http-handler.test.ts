...
describe('[Core/Presentation] CreateMentorProfileHttpHandler', () => {
  ...
  test('Request /mentor (POST) without being logged', async () => {});

  test('Request /mentor (POST) with bad or invalid header: authorization JWT token', async () => {});

  test('Request /mentor (POST). User account has been created (as mentor) and he is logged in.No profile created yet, invalid request payload', async () => {});

  test('Response /mentor (POST) when the request has been processed successfully', async () => {});
});
